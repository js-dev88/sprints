...
class T_e_client_cli extends Model {
	...
	public function __construct($id=null) {
		...
		if ($id == null) {
			...
			$st = db()->prepare("insert into t_e_client_cli(cli_mel, cli_motpasse, cli_pseudo, cli_civilite, cli_nom, cli_prenom, cli_telfixe, cli_telportable) values ('', '', '', 'M.', '', '', '', '') returning cli_id");
			$st->execute();
			$row = $st->fetch();
			$field = "cli_id";
		...
		if ($value != null) {
			if (property_exists(get_class($this), $varName)) {
				...
				if (isset($value->$id)) {
					...
					$st = db()->prepare("update t_e_client_cli set $fieldName=:val where cli_id=:id");
